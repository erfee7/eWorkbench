...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!secret) return jsonError(503, 'server_misconfigured');
    ...
    if (!updated) return jsonError(500, 'update_failed');
    ...
    return jsonErrorFromThrowable(err, code => ({ ok: false, error: code }), {
      logLabel: 'account:change-password',
